...
$( document ).ready( function() {
    ...
    */
    ...
    /**
  *
  * hooks to show client form popup
  *
  */

    $('*[data-action="show-client-popup"]').click(function () {

        var quoteKey = $(this).data('quote-key');
        if (typeof quoteKey != 'undefined' && quoteKey !== false) $.martanianShowClientPopup(quoteKey);

    });
    /**
  *
  * hooks to hide client popup
  *
  */

    $('#client-popup-close').click(function () {

        $.martanianHideClientPopup();
    ...
    */
    ...
    /**
*
* hide client form popup
* 
*/

    $.martanianHideClientPopup = function () {

        $('#client-popup #client-popup-content').removeClass('bounceInDown').addClass('bounceOutUp');
        setTimeout(function () {

            $('#client-popup #client-popup-background').fadeOut(300);
            $('#client-popup #client-popup-content').css({ 'top': '-10000px' }).removeClass('bounceOutUp');
            $('#client-popup #client-popup-content .client-form').hide();
            $('#client-popup #client-popup-content #client-popup-tabs li').removeClass('active');
    ...
    * 
    */
    $.martanianShowClientPopup = function (type) {

        var mode = windowWidth > 932 && windowHeight > 670 ? 'fixed' : 'absolute';
        var selectedClientForm = $('#client-popup #client-popup-content .client-form[data-client-form-for="' + type + '"]');

        $('#client-popup #client-popup-background').fadeIn(300);
        $('#client-popup #client-popup-content #client-popup-tabs li[data-client-tab-for="' + type + '"]').addClass('active');

        if (mode == 'absolute') $('html, body').animate({ 'scrollTop': '0' }, 300);

        selectedClientForm.show();
        setTimeout(function () {

            if (mode == 'fixed') {

                selectedClientForm.children('.client-form-background').css({ 'height': selectedClientForm.height() });
                $('#client-popup #client-popup-content').addClass('bounceInDown').css({ 'top': '50%', 'marginTop': -(selectedClientForm.height() / 2), 'height': selectedClientForm.height(), 'position': 'fixed' });
            }
                ...
                if (windowWidth > 932) {
                    ...
                    $('#client-popup #client-popup-content').addClass('bounceInDown').css({ 'top': '50px', 'marginTop': '0px', 'height': selectedClientForm.height(), 'position': 'absolute' });
                    selectedClientForm.children('.client-form-background').css({ 'height': selectedClientForm.height() });
                }

                else {

                    if (windowWidth > 582) $('#client-popup #client-popup-content').addClass('bounceInDown').css({ 'top': '50px', 'marginTop': '0px', 'height': selectedClientForm.height(), 'position': 'absolute' });
                    else {

                        $('#client-popup #client-popup-content').addClass('bounceInDown').css({ 'top': '50px', 'marginTop': '0px', 'height': selectedClientForm.height() + $('#client-popup-tabs').height() + 50, 'position': 'absolute' });
                    }
                }
            }
    ...
    */ 
     $.martanianShowLoginPopup = function( type ) {    
        ...
        var mode = windowWidth > 932 && windowHeight > 670 ? 'fixed' : 'absolute';
        var selectedLoginForm = $( '#login-popup #login-popup-content .login-form' );
        ...
        setTimeout( function() {
            ...
            if( mode == 'fixed' ) {
                ...
                $( '#login-popup #login-popup-content' ).addClass( 'bounceInDown' ).css({ 'top': '50%', 'marginTop': - ( selectedLoginForm.height() / 2 ), 'height': selectedLoginForm.height(), 'position': 'fixed' });
            }
    ...
    */
    ...
    $( '*[data-action="show-login-popup"]' ).click( function() {
	 ...
	 /**
    *
    * hooks to  logout the user
    *
    */
	$('*[data-action="show-logout"]').click(function(){
		var lgout=$('*[data-action="show-logout"]').attr('id');
		
		if(lgout=='gologout'){
		 signOut();
 isLogin=false;
 		loginFirst=true;
 	$('*[data-action="show-login-popup"]').show();
	$('*[data-action="show-logout"]').hide();
	$('*[data-action="show-client-popup"]').hide();
...
updateProfile
*/
$.uploadProfile=function(fb){
			var form_data = new FormData();                  
    form_data.append('file', files_data.file1);
	

$('#uploadP #cemail').val(fb.email);

   var collect=  $("#uploadP").serialize()+'&dl='+files_data.file1.name+'&mulkiya='+files_data.file2.name+'&emiratesid='+files_data.file3.name+'&fullname='+fb.name;
 
  
   $.ajax({
                url: 'test1.php', // point to server-side PHP script 
                ...
                success: function(php_script_response){
					...
					$('#uploadP').html('We will send you a quote withinan hours time');
                }
     });                          
    $.ajax({
                url: 'test1.php', // point to server-side PHP script 
	 ...
	 form_data = new FormData(); 
	  form_data.append('file', files_data.file2);
     ...
     $.ajax({
	  ...
	  form_data = new FormData(); 
	  form_data.append('file', files_data.file3);
     ...
     $.ajax({
         ...
         form_data = new FormData(); 
	  form_data.append('file', files_data.file4);
